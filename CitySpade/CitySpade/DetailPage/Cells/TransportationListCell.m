...
- (void)ConfigureCellWithItem:(BaseClass *)list {
    if ( list ) {
        self.list = list;
        if ( [_nameString isEqualToString:@"Colleges:"] ) {
            self.cellScreenHeight = @(list.colleagesCellHeight);
        } else if( [_nameString isEqualToString:@"Hottest spots:"] ) {
            self.cellScreenHeight = @(list.hottestSpotCellHeight);
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CitySpadeTransportationItem *transportationItem = _transporationsArray[indexPath.row];
    ...
    NSString *iconURLString = transportationItem.transtTypeIcon;
    
    NSURL *iconPath = [[CitySpadeModelManager sharedManager] imagePathForImageURL:iconURLString];
    ...
    if ( !iconPath ) {
        ...
        [[CitySpadeModelManager sharedManager] fetchImageForURL:iconURLString WithCompletionHandler:^ {
