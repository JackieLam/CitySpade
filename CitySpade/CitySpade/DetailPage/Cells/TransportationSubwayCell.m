...
- (void)ConfigureCellWithItem:(BaseClass *)list {
    if ( list ) {
        self.subwaysArray = list.subwayLines;
        ...
        self.cellScreenHeight = @(list.subwayCellHeight);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for ( NSUInteger i = 1; i < [subways count]; i++) {
        ...
        NSURL *iconPath = [[CitySpadeModelManager sharedManager] imagePathForImageURL:iconImageString];
        if ( !iconPath ) {
            UIActivityIndicatorView *indicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
            indicator.frame = iconFrame;
            [indicator startAnimating];
            [cell addSubview:indicator];
            [[CitySpadeModelManager sharedManager] fetchImageForURL:iconImageString
