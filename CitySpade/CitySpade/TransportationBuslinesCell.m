...
- (void)ConfigureCellWithItem:(CitySpadeListsModel *)list {
    ...
    NSArray *busArray = dictionary[@"bus_lines"];
    if ( [busArray count] != 0 ) {
        _nameLabel.hidden = NO;
        BOOL isFirstBus = YES;
        for (NSDictionary *bus in busArray) {
            if ( isFirstBus ) {
                isFirstBus = NO;
                _busLabel.text = [_busLabel.text stringByAppendingString:[NSString stringWithFormat:@" %@", bus[@"line_name"]]];
            } else {
                _busLabel.text = [_busLabel.text stringByAppendingString:[NSString stringWithFormat:@", %@", bus[@"line_name"]]];
            }
        }
